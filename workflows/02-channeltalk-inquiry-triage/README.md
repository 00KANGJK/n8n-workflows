...
> **알림·라우팅까지 전부 채널톡 안에서 완결** — 운영자가 제품을 벗어나지 않도록 설계했습니다.
...
- FAQ로 풀리면 **봇이 자동 응답**, 사람이 필요하면 **태그 부착 + 담당 배정 + (긴급 시) 우선순위 '높음' 설정 → 채널톡 자체 알림**

> **왜 채널 네이티브로 통일했나 (설계 판단)**
> 외부 알림 도구(Slack 등)를 붙이지 않고, **태그·배정·우선순위·알림을 모두 채널톡 안에서** 처리했습니다.
> ① 상담원이 채널톡을 벗어나지 않고 처리 → 실제 운영 동선에 맞음
> ② 연동이 줄어 구조가 단순하고 장애 지점이 적음 (모두 하나의 Open API)
> ③ 긴급 건은 **우선순위 '높음' + 담당 배정**만 하면 채널톡이 담당자에게 **자체 알림** → 별도 알림 노드 불필요
>
...
flowchart TD
    ...
    G --> H{긴급도 높음?}
    H -- 예 --> I[상담 우선순위 '높음' 설정<br/>→ 채널톡 자체 알림으로 담당자 인지]
    H -- 아니오 --> J[대기: 상담원 처리]
    F --> K[(로그 · 지표 적재)]
    G --> K
...
| 6 | **HTTP Request — 우선순위 설정** | 긴급 건(긴급도 3)에 상담 **우선순위 '높음'** 설정 → 채널톡이 담당자에게 **자체 알림** |
| 7 | **Error Trigger / Wait** | 429 응답 시 `x-ratelimit-reset`까지 대기 후 재시도, 실패 건은 재시도 큐 |

> 별도 외부 알림 노드(Slack 등)가 없습니다. 긴급 알림은 **6번(우선순위 설정) + 5b(담당 배정)** 만으로 채널톡 네이티브 알림이 트리거됩니다.
...
| 우선순위 설정 | 상담 우선순위(예: 높음) 업데이트 → **채널톡 자체 알림** 트리거 |

> 정확한 요청/응답 스키마와 엔드포인트 이름은 [api-doc.channel.io](https://api-doc.channel.io) 최신 버전을 기준으로 확인합니다.
> (우선순위/내부 알림 API가 공개 범위에 없다면, **담당 배정만으로도** 채널톡의 기본 배정 알림이 담당자에게 전달됩니다.)
...
이 JSON을 기준으로 태그(`type`)·라우팅·자동응답 여부(`faq_answerable`)·우선순위(`urgency`)가 분기됩니다.

---

## 🛟 에러 핸들링
- 모든 HTTP 노드에 실패 분기 연결 → 실패 시 **운영자 알림(이메일 또는 채널톡 팀 메신저) + 재시도 큐**로 즉시 인지
...
- 긴급 건은 **우선순위·배정 기반 채널톡 네이티브 알림**으로 누락 없이 담당자에게 도달
...
- (옵션) 필요 시 Slack·이메일 등 외부 채널로 **긴급 알림 미러링** — 기본은 채널톡 네이티브로 완결
